...
export async function searchCoreApi(
  query: string,
  field = ""
  ...
  try {
    ...
    if (process.env.NODE_ENV === "development" && !process.env.CORE_API_KEY) {
      ...
      cache[cacheKey] = {
        data: { papers: mockData, total: mockData.length },
        timestamp: Date.now(),
      };
    ...
    if (!response.ok) {
      if (response.status === 429) {
        ...
        cache[cacheKey] = {
          data: { papers: fallbackData, total: fallbackData.length },
          timestamp: Date.now(),
        };
...
function getMockPapers(query: string, field = ""): Paper[] {
  ...
  if (field) {
    ...
    papers = papers.map((paper) => ({
      ...paper,
      topics:
        paper.id.includes("mock-2") ||
        paper.id.includes("mock-5") ||
        paper.id.includes("mock-8")
